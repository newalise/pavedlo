...
export class CatalogManageComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit(): void {
    ...
    this.optionsForm.controls['quantity'].valueChanges.subscribe(
      (value) => {
  ...
  public _updateQuantityProduct(product: MaiCatalogItemModel) {
    this.subscriptions.add(this.catalogManageService.updateQuantityProductById(product.id, this.optionsForm.getRawValue().quantity)
      .subscribe((data) => {
        window.location.reload();
      }));
  }
